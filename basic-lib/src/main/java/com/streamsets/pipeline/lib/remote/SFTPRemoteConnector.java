...
/**
 * Handles all of the logic about connecting to an SFTP server, including config
 * verification. Subclasses can get this functionality for free by simply
 * calling {@link #initAndConnect(List, ConfigIssueContext, URI, Label, Label)}
...
public abstract class SFTPRemoteConnector extends RemoteConnector {
	...
	@Override
	public void initAndConnect(List<Stage.ConfigIssue> issues, ConfigIssueContext context, URI remoteURI,
		...
		sshConfig.setCompressionFactories(Arrays.asList(new DelayedZlibCompression.Factory(), new ZlibCompression.Factory(),
			...
			if (knownHostsFile != null) {
				if (knownHostsFile.exists() && knownHostsFile.isFile() && knownHostsFile.canRead()) {
					try {
						...
						issues.add(context.createConfigIssue(credGroup.getLabel(), CONF_PREFIX + "knownHosts", Errors.REMOTE_03,
								knownHostsFile));
					}
				} else {
					issues.add(context.createConfigIssue(credGroup.getLabel(), CONF_PREFIX + "knownHosts", Errors.REMOTE_03,
				...
				issues
						.add(context.createConfigIssue(credGroup.getLabel(), CONF_PREFIX + "strictHostChecking", Errors.REMOTE_04));
			}
		} else {
		...
		try {
			...
			case PRIVATE_KEY:
				String privateKeyPassphrase = resolveCredential(remoteConfig.privateKeyPassphrase,
						CONF_PREFIX + "privateKeyPassphrase", issues, context, credGroup);
				...
				case FILE:
					...
					if (!privateKeyFile.exists() || !privateKeyFile.isFile() || !privateKeyFile.canRead()) {
						issues.add(context.createConfigIssue(credGroup.getLabel(), CONF_PREFIX + "privateKey", Errors.REMOTE_05,
					...
					break;
				case PLAIN_TEXT:
					String privateKeyPlainText = resolveCredential(remoteConfig.privateKeyPlainText,
							CONF_PREFIX + "privateKeyPlainText", issues, context, credGroup);
					sshClientRebuilder.setKeyPlainText(privateKeyPlainText);
					if (privateKeyPassphrase != null && !privateKeyPassphrase.isEmpty()) {
						keyProvider = sshClient.loadKeys(privateKeyPlainText, null,
				...
				// No need to bother trying this if the keyProvider is still null - issues were
				// already found
				if (keyProvider != null) {
			...
			issues.add(context.createConfigIssue(credGroup.getLabel(),
					CONF_PREFIX + (remoteConfig.privateKeyProvider.equals(PrivateKeyProvider.PLAIN_TEXT) ? "privateKeyPlainText"
							: "privateKey"),
					Errors.REMOTE_10, e.getMessage(), e));
		}
		...
		if (issues.isEmpty()) {
			...
			try {
				...
				this.sftpClient = new ChrootSFTPClient(sftpClientRaw, remotePath, remoteConfig.userDirIsRoot,
						remoteConfig.createPathIfNotExists);
				this.sftpClient.ls();
			} catch (IOException e) {
				LOG.error(Errors.REMOTE_11.getMessage(), remoteURI.toString(), e.getMessage(), e);
				issues.add(context.createConfigIssue(remoteGroup.getLabel(), CONF_PREFIX + "remoteAddress", Errors.REMOTE_11,
						remoteURI.toString(), e.getMessage(), e));
			}
		}
	}
	...
	public void verifyAndReconnect() throws StageException {
		...
		while (!done && retryCounter < MAX_RETRIES) {
			try {
				...
				// ls can fail due to session is down, a timeout, etc; so try getting a new
				// connection
